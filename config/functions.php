<?php
    ...
    function generateToken() {
        ...
        $parame =array(
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_HTTPHEADER => array("accept: */*", 'Authorization: '.'Basic '.base64_encode(VISA_USER.":".VISA_PWD)));
    ...
    function generateSesion($amount, $token) {
        $session = array(
            'amount' => $amount,
            'antifraud' => array('clientIp' => $_SERVER['REMOTE_ADDR'],'merchantDefineData' => array('MDD4' => "mail@domain.com",'MDD33' => "DNI",'MDD34' => '87654321'),
            ),
            'channel' => 'web'
        );
